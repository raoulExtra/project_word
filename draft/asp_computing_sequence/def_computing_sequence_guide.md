---
id: def_computing_sequence_guide
name: Computing Sequence Guide
kind: documentation
status: active
domain: computing
subdomain: efficiency_binary
---

# Computing Sequence Guide

## Overview
The computing sequence defines the canonical order for introducing computational concepts, reflecting the dependency hierarchy of how computation is built up layer by layer.

## Sequence
1. **bit** - Fundamental unit of information
2. **boolean** - Semantic meaning of bits (true/false)
3. **byte** - Group of 8 bits
4. **word** - CPU's native processing unit
5. **integer** - Word interpreted as number
6. **arithmetic operations** - Math on integers
7. **logic operations** - Boolean/bit manipulation
8. **registers** - Fast CPU storage
9. **memory** - Byte-addressable storage
10. **instruction** - CPU operation commands

## Dependency Principle
Each concept depends on all previous concepts. This is the minimal dependency chain for building computational systems.

## See Also
- `def_bit.md` - Bit definition
- `def_boolean.md` - Boolean definition
- `def_byte.md` - Byte definition
- `def_word.md` - Word definition
- `def_integer.md` - Integer definition
- `def_arithmetic_operations.md` - Arithmetic operations
- `def_logic_operations.md` - Logic operations
- `def_registers.md` - Registers
- `def_memory.md` - Memory
- `def_instruction.md` - Instructions